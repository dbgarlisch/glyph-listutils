...
proc main {} {
    ...
    set cnt 0
    test_lproduct_foreach "test [incr cnt]" $s1 $s2
    test_lproduct_foreach "test [incr cnt]" $s1 $s3
...
proc test_lproduct_foreach { desc args } {
    puts {}
    puts "foreach ${desc}: [list {*}$args]"
    set cnt 0
    pw::listutils lproduct foreach result {*}$args {
...
proc test_lmutate_foreach { items } {
    puts {}
    puts "lmutate foreach of [list $items]"
    set cnt 0
    pw::listutils lmutate foreach result $items {
...
proc test_lmutate_get { items } {
    ...
    puts {}
    puts "lmutate get of [list $items]"
    foreach r $ret {
	puts "  [list $r]"
    }
}
